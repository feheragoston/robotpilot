...
nokia_server::nokia_server(std::string ipaddr)
{
    //ctor
  ...
  this->address.sin_addr.s_addr = inet_addr(ipaddr.c_str());
...
void nokia_server::set_location(double x, double y, double phi)
{
phi = phi * 180.0 / M_PI;
std::string out = "<location x=\"" + this->from_int(x) + "\" y=\"" + this->from_int(y) + "\" phi=\"" + this->from_int(phi) + "\" />\n";
this->current_xml += out;
if(this->instant_send) this->do_send();
}
void nokia_server::set_speed(double left_wheel, double right_wheel)
{
...
{
std::string out;
if(state == OPENED) out = "<state name=\"opened\" />\n";
else out = "<state name=\"closed\" />\n";
this->current_xml += out;
if(this->instant_send) this->do_send();
}

void nokia_server::set_opponent(std::string name, double x, double y, double phi, double r)
{
phi = phi * 180.0 / M_PI;
std::string out = "<opponent name=\"" + name +"\" x=\"" + this->from_int(x) + "\" y=\"" + this->from_int(y) + "\" phi=\"" + this->from_int(phi) + "\" r=\"" + this->from_int(r) + "\" />\n";
this->current_xml += out;
if(this->instant_send) this->do_send();
}
...
std::string types;
if(type == NONE) types = "";
else if(type == INFO) types = "info";
else if(type == WARNING) types = "warning";
else if(type == ERROR) types = "error";
...
void nokia_server::do_send()
{
  ...
  if(this->current_xml.size() == 0) return;
