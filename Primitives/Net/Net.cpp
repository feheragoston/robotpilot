...
bool Net::Send(void* c, size_t size) {
	if (s > 0) {
		if (send(s, &size, sizeof(size_t), 0) >= 0 && send(s, c, size, 0) >= 0) {
			return true;
		}
	}
	return false;
...
int Net::Receive(void* c, size_t size, long int useconds) {
	if (s > 0) {
		struct timeval tv;
		fd_set rfd;
		...
		default:
			if (FD_ISSET(s, &rfd)) {
				int size_read = recv(s, c, sizeof(size_t), 0);
				size_t msg_len = *((size_t*) c);
