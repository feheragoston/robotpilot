...
void node_Power::INIT_PARAM(void){
	...
	SET_U16(&(msg.data[4]), POWER_MAIN_VOLTAGE_STOPBUTTON_FALLING_TIME_US);
	SET_U16(&(msg.data[6]), POWER_MAIN_VOLTAGE_STOPBUTTON_RISING_TIME_US);

	SET_U16(&(msg.data[8]), POWER_A_TO_ANALOG(POWER_MAIN_CURRENT_SHUTDOWN_LEVEL));
	SET_U16(&(msg.data[10]), POWER_A_TO_ANALOG(POWER_ACT_CURRENT_SHUTDOWN_LEVEL));

	SET_U16(&(msg.data[12]), POWER_V_TO_ANALOG(POWER_ACCU_VOLTAGE_OK_LEVEL));

	SET_U16(&(msg.data[14]), POWER_V_TO_ANALOG(POWER_CHARGE_VOLTAGE_READY_LEVEL));
	SET_U16(&(msg.data[16]), POWER_V_TO_ANALOG(POWER_CHARGE_VOLTAGE_PLUGGED_IN_LEVEL));
